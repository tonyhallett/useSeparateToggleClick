...
export type ShouldToggleClick = (isIconClick:boolean, isExpanding:boolean) =>boolean
export type ToggleReason = 'expand'|'collapse'|'nochange';
export type ShouldToggleNonClick = (reason:ToggleReason,newOrRemoved:Array<string>)=>boolean
...
export const useSeparateToggleClick = (
  ...
  const lastIconClickManager = React.useRef<ILastIconClickManager>(new LastIconClickTimestampManager());
  const IconWrapper = function IconClickedEventAttacher(props:{children:any}) {
      ...
      (evt:React.ChangeEvent<{}>, newExpanded:string[]) => {
        setExpanded(oldExpanded => {
          ...
          if (evt.nativeEvent && evt.nativeEvent.type === "click") {
            const isIconClick = lastIconClickManager.current.isLastIconClick(evt);
            const toggleClick = shouldToggleClick(
              isIconClick,
              isExpanding,
            );
            if (toggleClick) {
          ...
          const toggleNonClick = shouldToggleNonClick(
            isExpanding?'expand':newExpanded.length === oldExpanded.length ? 'nochange':'collapse',
            newOrRemoved,
          );
